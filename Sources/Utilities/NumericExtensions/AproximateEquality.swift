...
extension FloatingPoint {
    ...
    /**
     ...
     Adapted from [Swift Evolution Proposal 0259](https://github.com/apple/swift-evolution/blob/master/proposals/0259-approximately-equal.md)
     ...
     - Parameters:
       ...
       - relativeTolerance: The relative tolerance to use
             for the comparison. Should be in the range
             (.ulpOfOne, 1).
       - absoluteTolerance: The absolute tolerance for the comparison.
    
     - Returns: `true` if `self` and `other` are approximately equal
           based on at least **one** of the tolerances;
           otherwise `false`.
     */
    @inlinable
    public func isAlmostEqual(
        ...
        assert(
            relativeTolerance >= .ulpOfOne && relativeTolerance < 1,
        ...
        return abs(self - other) <= max(scale * relativeTolerance, absoluteTolerance)
    ...
    @usableFromInline
    internal func rescaledAlmostEqual(
        to other: Self,
        relativeTolerance: Self,
        absoluteTolerance: Self
    ) -> Bool {
        // NaN is considered to be not approximately equal to anything,
        // not even itself.
        ...
        if self.isInfinite {
            ...
            return scaledSelf.isAlmostEqual(
                to: scaledOther,
                relativeTolerance: relativeTolerance,
                absoluteTolerance: absoluteTolerance
            )
        ...
        return other.rescaledAlmostEqual(
            to: self,
            relativeTolerance: relativeTolerance,
            absoluteTolerance: absoluteTolerance
        )
